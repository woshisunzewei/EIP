...
namespace EIP.Common.Pay.WeiXin.business
{
    public class NativePay
    {
        ...
        * @return 模式二URL
        */
        public string GetPayUrl(string productId, string realPrice, string body)
        {
            //Log.Info(this.GetType().ToString(), "Native pay mode 2 url is producing...");

            WxPayData data = new WxPayData();
            data.SetValue("body", body);//商品描述
            data.SetValue("attach", "译宝联科技");//附加数据
            data.SetValue("out_trade_no", productId);//随机字符串
            data.SetValue("total_fee", realPrice);//总金额
