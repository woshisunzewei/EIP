...
(function ($) {
    ...
    $.ligerDefaults.TabString = {
        closeMessage: "关闭当前页",
        closeMessageIcon: "table",
        closeOtherMessage: "关闭其他",
        closeOtherMessageIcon: "tables",
        closeAllMessage: "关闭所有",
        closeAllMessageIcon: "tables-stacks",
        reloadMessage: "刷新",
        reloadMessageIcon: "arrow-circle-225-left"
    };
    ...
    },
    {
        text: p.reloadMessage, id: 'reload', icon: p.reloadMessageIcon, click: function () {
            ...
            if (p.onTabId != null) {
                g.tab.links.prependTo($("#" + p.onTabId));
            } else {
            ...
            $("> div", g.tab.content).each(function (i, box) {
                ...
                if (contentitem.attr("icon")) {
                    $("> a", li).addClass("l-icon-" + contentitem.attr("icon"));
                    $("> a", li).css("padding-left", "18px");
                }
                ...
                if (contentitem.find("iframe").length > 0) {
                    ...
                    if (iframe[0].readyState != "complete") {
                        if (contentitem.find(".l-tab-loading:first").length == 0)
                            contentitem.prepend("<div class='l-tab-loading' style='display:block;'>界面加载中,请稍等...</div>");
        ...
        addTabItem: function (options) {
            ...
            var contentitem = $("<div class='l-tab-content-item'><div class='l-tab-loading' style='display:block;'>界面加载中,请稍等...</div><iframe frameborder='0'></iframe></div>");
            ...
            //添加图标开始-孙泽伟
            if (options.icon) {
                $("a", tabitem).addClass("l-icon-" + options.icon);
                $("a", tabitem).css("padding-left", "18px");
            }
            //添加图标结束-孙泽伟
            if (text == undefined) text = tabid;
            if (height) contentitem.height(height);
