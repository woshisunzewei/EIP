...
function GooFlow(bgDiv, property) {
    ...
    if (property.haveHead) {
        ...
        if (property.headLabel) {
            tmp += "<label title='" + (property.initLabelText || "新建流程") + "' "
              + (GooFlow.prototype.color.main ? "style='background:" + GooFlow.prototype.color.main + "'" : "") + ">" + (property.initLabelText || "新建流程") + "</label>";
        }
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            This.addNode(UtilNewGuid(), { name: "审批", left: X, top: Y, type: This.$nowType });
            This.$max++;
        });
        //划线或改线时用的绑定
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            if (lineStart && !This.$mpTo.data("p")) {
                ...
                This.addLine(UtilNewGuid(), { from: lineStart.id, to: this.id, name: "" });
                This.$max++;
            }
            else {
                if (lineStart) {
