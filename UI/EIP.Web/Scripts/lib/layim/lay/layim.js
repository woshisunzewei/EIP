...
; !function (win, undefined) {
    ...
    var config = {
        ...
        right: -232,
        api: {
            friend: '', //好友列表接口http://localhost:58576/WebIm/GetFriends
            group: 'Scripts/lib/layim/group.json', //群组列表接口 
            chatlog: 'Scripts/lib/layim/chatlog.json', //聊天记录接口
            groups: 'Scripts/lib/layim/groups.json', //群组成员接口
            sendurl: '' //发送消息接口
        },
        user: { //当前用户信息
            name: username,
            face: 'Scripts/lib/layim/images/1.png'
        },
        ...
        solutionHub = $.connection.webImHub,
        editor,
        onlinenum = 0;//在线人数;
    ...
    xxim.popchat = function (param) {
        ...
        log.html = '<div class="layim_chatbox" id="layim_chatbox">'
                ...
                //+ '    </div>'
                + '    <textarea  id="layim_write" name="layim_write"></textarea>'
        ...
        if (config.chatings >= 1) {
            DialogTipsMsgWarn("当前已有聊天窗口,请关闭后重试", 3000);
            return;
        }
        ...
        KindEditor.ready(function (K) {
            editor = K.create('textarea[name="layim_write"]', {
                basePath: '/scripts/lib/kindeditor/',
            ...
            $(".ke-statusbar").hide();
            $(".ke-container").css("margin-left", "0.5px");
            $(".xubox_layer").css("z-index", "9999");
        });
    };
        ...
        log.send = function () {
            ...
            if (data.content.replace(/\s/g, '') === '') {
                ...
                */
                ...
                //node.imwrite.val('').focus();
                editor.html('');
                $(".ke-edit-iframe").contents().find(".ke-content").focus();
                log.imarea.scrollTop(log.imarea[0].scrollHeight);

                // 调用服务端sendPrivateMessage方法来转发消息
                solutionHub.server.sendPrivateMessage(xxim.nowchat.id, data.content);
            }
    ...
    xxim.event = function () {
        ...
        // 连接IM服务器成功
        // 主要是更新在线用户
        solutionHub.client.onConnected = function (id, userName, allUsers) {
            var node = xxim.node, myf = node.list.eq(0), str = '', i = 0;
            myf.addClass('loading');
            onlinenum = allUsers.length;
            if (onlinenum > 0) {
                str += '<li class="xxim_parentnode  xxim_liston">'
                     + '<h5><i></i><span class="xxim_parentname">在线用户</span><em class="xxim_nums">（' + onlinenum + '）</em></h5>'
                     + '<ul id="xxim_friend_list" class="xxim_chatlist">';
                for (; i < onlinenum; i++) {
                    str += '<li id="userid-' + allUsers[i].UserID + '" data-id="' + allUsers[i].ConnectionId + '" class="xxim_childnode" type="one"><img src="' + allUsers[i].Face + '"  class="xxim_oneface"><span  class="xxim_onename">' + allUsers[i].UserName + '</span><em class="xxim_time">' + allUsers[i].LoginTime + '</em></li>';
                }
                str += '</ul></li>';
                myf.html(str);
            } else {
        ...
        //新用户上线
        solutionHub.client.onNewUserConnected = function (id, userId, userName, loginTime) {
            onlinenum = onlinenum + 1;
            $(".xxim_nums").html("（" + onlinenum + "）");
            var myf = $('#xxim_friend_list'), str = '';
            str += '<li id="userid-' + userId + '" data-id="' + id + '" class="xxim_childnode" type="one"><img src="/Content/Images/001.jpg?' + userId + '"  class="xxim_oneface"><span  class="xxim_onename">' + userName + '(' + ')</span><em class="xxim_time">' + loginTime + '</em></li>';
            myf.append(str);
        };

        //用户离线
        solutionHub.client.onUserDisconnected = function (id, userName) {
            onlinenum = onlinenum - 1;
            $(".xxim_nums").html("（" + onlinenum + "）");
            $("#xxim_friend_list li[data-id=" + id + "]").remove();

            //弹出提示框进行提示
            art.dialog.notice({
                title: '离线提示',
                width: 220,// 必须指定一个像素宽度值或者百分比，否则浏览器窗口改变可能导致artDialog收缩
                content: userName + '<br/>离线了',
                icon: 'face-smile',
                time: 3
            });
        };
        ...
        //发送消息时，对方已不在线
        solutionHub.client.absentSubscriber = function () {
            $.gritter.add({
                title: "系统提醒",
        ...
        // 接收消息
        solutionHub.client.receivePrivateMessage = function (fromUserId, userName, message) {
            var node = xxim.node, log = {}, othis = $("#xxim_friend_list li[data-id=" + fromUserId + "]");
            //聊天模版
            log.html = function (param, type) {
                return '<li class="' + (type === 'me' ? 'layim_chateme' : '') + '">'
                    + '<div class="layim_chatuser">'
    ...
    xxim.view = (function () {
        ...
        $.connection.hub.start().done(function () {
            solutionHub.server.connect(userid, username); // 调用服务端connect方法
        });
