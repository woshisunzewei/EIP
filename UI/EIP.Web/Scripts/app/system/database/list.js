...
//操作:新增
function add() {
    ArtDialogOpen("/System/DataBase/Edit", "新增系统数据库", true, 400, 580);
}
...
function edit() {
    ...
    GridIsSelect($grid, function () {
        var info = GridGetSingSelectData($grid);
        ArtDialogOpen("/System/DataBase/Edit?id=" + info.DataBaseId, "修改系统数据库-" + info.Name, true, 400, 580);
    });
}
...
function del() {
    ...
    GridIsSelect($grid, function () {
        ArtDialogConfirm(Language.common.deletemsg, function () {
            UtilAjaxPostWait(
                "/System/DataBase/DeleteDataBase",
                { id: GridGetSingSelectData($grid).DataBaseId },
                perateStatus
            );
        });
    });
