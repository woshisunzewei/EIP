...
function initGird() {
    $grid = $("#list").jgridview(
            {
                url: "/System/Group/GetGroupByOrganizationId",
                multiselect: false,
                datatype: "json", //取数格式json,xml等
                colModel: [
                        { name: "GroupId", hidden: true },
                        { label: "名称", name: "Name", width: 150, fixed: true },
                        { label: "归属组织", name: "OrganizationName", width: 150, fixed: true },
                        { label: "创建类型", name: "BelongToName", width: 60, align: "center", fixed: true },
                        { label: "创建人", name: "BelongUserName", width: 80, align: "center", fixed: true, search: false },
...
function getGridData() {
    ...
    UtilAjaxPost(
                "/System/Group/GetGroupByOrganizationId",
                 { Id: pId },
                function (data) {
...
function add() {
    ...
    ArtDialogOpen("/System/Group/Edit?organizationId=" + organizationId + "&organizationId=" + organizationName, "新增组", true, 300, 590);
}
...
function edit() {
    ...
    GridIsSelect($grid, function () {
        ...
        ArtDialogOpen("/System/Group/Edit?groupId=" + info.GroupId, "修改组-" + info.Name, true, 300, 590);
    });
}
...
function del() {
    ...
    GridIsSelect($grid, function () {
        ArtDialogConfirm(Language.common.deletemsg, function () {
            UtilAjaxPostWait(
                 "/System/Group/DeleteGroup",
                { id: GridGetSingSelectData($grid).GroupId },
                perateStatus
            );
        });
    });
...
function groupUser() {
    ...
    GridIsSelect($grid, function () {
        ...
        ArtDialogOpen("/Common/UserControl/PrivilegeMasterUser?" +
            ...
            "privilegeMasterValue=" + info.GroupId,
            "维护组用户-" + info.Name, true, 450, 830);
    });
}
...
function menuPermission() {
    ...
    GridIsSelect($grid, function () {
        var info = GridGetSingSelectData($grid);
        ArtDialogOpen("/System/Permission/Menu?privilegeMasterValue=" + info.GroupId + "&privilegeMaster=" + Language.privilegeMaster.group, "模块权限授权-" + info.Name, true, 390, 380);
    });
}
...
function functionPermission() {
    ...
    GridIsSelect($grid, function () {
        var info = GridGetSingSelectData($grid);
        ArtDialogOpen("/System/Permission/Function?privilegeMasterValue=" + info.GroupId + "&privilegeMaster=" + Language.privilegeMaster.group, "模块按钮权限授权-" + info.Name, true, 490, 810);
    });
}
...
function fieldPermission() {
    ...
    GridIsSelect($grid, function () {
        var info = GridGetSingSelectData($grid);
        ArtDialogOpen("/System/Permission/Field?privilegeMasterValue=" + info.GroupId + "&privilegeMaster=" + Language.privilegeMaster.group, "字段权限授权-" + info.Name, true, 490, 810);
    });
}
...
function dataPermission() {
    ...
    GridIsSelect($grid, function () {
        var info = GridGetSingSelectData($grid);
        ArtDialogOpen("/System/Permission/Data?privilegeMasterValue=" + info.GroupId + "&privilegeMaster=" + Language.privilegeMaster.group, "数据权限授权-" + info.Name, true, 490, 810);
    });
}
