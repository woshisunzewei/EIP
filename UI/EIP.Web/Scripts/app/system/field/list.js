...
function initGird() {
    $grid = $("#list").jgridview(
    {
        ...
        colModel: [
            ...
            { label: "显示名称", name: "Label", width: 100, fixed: true },
            ...
            { label: "列宽", name: "Width", width: 100, fixed: true },
            { label: "对齐方式", name: "Align", width: 50, fixed: true },
            { label: "格式化", name: "Formatter", width: 80 },
            { label: "排序方式", name: "Sorttype", width: 80 },
            { label: "排序", name: "OrderNo", align: "center", index: "field.OrderNo", width: 50, fixed: true },
            { label: "冻结", name: "IsFreeze", index: "field.IsFreeze", formatter: "YesOrNo", width: 50, align: "center", fixed: true },
            { label: "显示", name: "Hidden", width: 50, fixed: true, formatter: "YesOrNo" },
            { label: "导出/打印", formatter: "YesOrNo", name: "CanbeDerive", width: 60, fixed: true },
            { label: "宽度固定", formatter: "YesOrNo", name: "Fixed", width: 50, fixed: true },
            { label: "备注", name: "Remark", index: "field.Remark", width: 100, fixed: true }
        ],
        height: $("#listcenter").height() - 51,
        sortname: "menu.Name,field.OrderNo",
        sortorder: "asc",
        postData: { IsShowHidden: true },
        onSelectRow: function (rowid) {
            var rowDatas = $grid.jqGrid('getRowData', rowid);
            getById(Language.privilegeAccess.field, rowDatas.FieldId);
        },
...
function initTreeData() {
    UtilAjaxPost("/System/Menu/GetHaveFieldPermissionMenu", null, function (data) {
        zNodes = data;
        initTree();
    });
}
...
function add() {
    ...
    var addUrl = "/System/Field/Edit";
    if (treeNode.length !== 0) {
        addUrl += "?menuId=" + treeNode[0].id + "&menuName=" + treeNode[0].name;
    }
    ArtDialogOpen(addUrl, "新增字段", true, 400, 580);
}
...
function edit() {
    ...
    GridIsSelect($grid, function () {
        ...
        ArtDialogOpen("/System/Field/Edit?id=" + info.FieldId, "修改字段-" + info.Label, true, 400, 580);
    });
}
...
function del() {
    ...
    GridIsSelect($grid, function () {
        ArtDialogConfirm(Language.common.deletemsg, function () {
            UtilAjaxPostWait(
                "/System/Field/DeleteField",
                { id: GridGetSingSelectData($grid).FieldId },
                perateStatus
            );
        });
    });
